import { OpenAIResponseBody, Choice, Message } from '../../openai/response/types';
import { AntResponseBody, ContentBlock } from './types';

export function toOpenAI(response: AntResponseBody): OpenAIResponseBody {
	const content = response.content.map((block) => blockToString(block)).join('');
...
function mapStopReason(reason: AntResponseBody['stop_reason']): Choice['finish_reason'] {
	switch (reason) {
		case 'end_turn':
		case 'stop_sequence':
			return 'stop';
