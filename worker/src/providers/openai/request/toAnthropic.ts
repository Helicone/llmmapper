...
export function toAnthropic(openAIBody: OpenAIRequestBody): AntRequestBody {
	if (!openAIBody.max_tokens) {
		throw new Error('max_tokens is required to map to Anthropic');
	}

	if (openAIBody.frequency_penalty) {
		throw new Error('frequency_penalty is not supported');
	}
	...
	const antBody: AntRequestBody = {
		...
		top_k: openAIBody.presence_penalty !== undefined ? openAIBody.presence_penalty : 40,
		stop_sequences: Array.isArray(openAIBody.stop) ? openAIBody.stop : openAIBody.stop ? [openAIBody.stop] : [],
...
function extractSystemMessage(messages: OpenAIRequestBody['messages']): { messages: OpenAIRequestBody['messages']; system?: string } {
...
function mapMessages(messages: OpenAIRequestBody['messages']): AntRequestBody['messages'] {
				...
				if (item.type === 'text') {
					...
					return {
						...
						source: {
							...
							media_type: url.startsWith('data:') ? url.split(';')[0].split(':')[1] : `image/${url.split('.').pop()}`,
